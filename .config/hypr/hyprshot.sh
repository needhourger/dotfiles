...
function save_geometry() {
    ...
    if [ $CLIPBOARD -eq 0 ]; then
        ...
        # 唤起 satty 进行编辑，输出覆盖原文件
        # 如果 satty 正常退出 (完成编辑)
        if satty --filename "$output" --output-filename "$output"; then
            # 将编辑后的最终图片写入剪贴板
            ...
            # 如果你在 satty 中按 Esc 取消了，自动删除残留的截图并静默退出
            rm -f "$output"
        fi

    # 3. 纯剪贴板模式 (--clipboard-only)
    else
        # 创建一个临时文件供 satty 读写
        local tmp_file=$(mktemp /tmp/hyprshot_satty_XXXXXX.png)
        grim -g "${geometry}" "$tmp_file"
        
        # 唤起 satty 进行编辑
        if satty --filename "$tmp_file" --output-filename "$tmp_file"; then
            # 编辑完成后，写入剪贴板并发送通知
            wl-copy --type image/png < "$tmp_file"
            send_notification $output
        fi
        
        # 无论是否取消，强制清理临时文件，防止爆满 /tmp
        rm -f "$tmp_file"
    fi
}

function save_geometry_bak() {
    local geometry="${1}"
    local output=""
